...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE,fig.path = "figures/", fig.keep = "all", keep_md = TRUE)
```
...
# The libraries loaded for this assigment
library(dplyr)
library(chron)
library(ggplot2)
```
...
```{r LoadFile}
# Reading input file after downloading and unzippping
...
ggplot(mean.by.weekdays, aes(x=interval, y=MeanSteps)) + geom_line(linetype = 1, size = 0.3, aes(color = factor(week.days))) + facet_grid(week.days ~ .) + ggtitle("Comparing the average number of steps taken per 5-minute \n interval on weekdays and weekends") + xlab("5 minute intervals") + ylab("Average Number of Steps taken")
```
